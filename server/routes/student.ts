    ...
    course.units.forEach((unit) => {
      if (unit.sortOrder && unit.cards) {
        ...
        unit.cards = unit.cards.sort((a, b) => {
          return (
            sortOrder.findIndex((d) => d === a.id) -
            sortOrder.findIndex((d) => d === b.id)
          );
        });
      }
    });
            ...
            Activity.create({
              ...
              studentUnitProgress(card.unit.id, studentId).then((progress) => {
                if (progress.unitCompleted) {
                  incrementCompletedUnits(card.unit.course.id, studentId).then(
                    (result) => {
...
app.get("/student/card/:cardId/:format", (req, res) => {
  ...
  Promise.all([
    ...
    if (courseIds.indexOf(card.unit.courseId) === -1) {
      res.status(401).send({
        message: "Unauthorized: Student does not own Card #" + cardId,
      });
      return;
    }
...
app.get("/student/progress", (req, res) => {
  Student.scope("public")
    .findByPk(req.user.student.id, { include: [Course] })
    .then((student) => {
