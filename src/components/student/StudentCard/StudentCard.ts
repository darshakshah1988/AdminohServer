...
export class StudentCard extends Vue {
  ...
  get parsedQuestions() {
    return this.activeStudentCard.quiz
      ? JSON.parse(this.activeStudentCard.quiz)
      : [];
  }
  ...
  handleFinish() {
    ...
    const courseIndex = this.student.courses.findIndex(
      (course) => course.id === courseId
    );
    const unitIndex = this.student.courses[courseIndex].units.findIndex(
      (unit) => unit.id === unitId
    );
    ...
    if (
      !this.activeStudentCard.nextCardId &&
      this.activeStudentCard.nextUnitId
    ) {
  ...
  watchSponsor(newVal, oldVal) {
    ...
    if (newVal && newVal.length > 0) {
      ...
      store
        .dispatch("getStudentSponsorLogo", {
          filename: newVal[0].logo,
        })
        .then((url) => {
          this.logo = url;
        });
