...
export const actions = {
  sendDowngradeRequest(context, payload) {
    return axios
      .post(BASE_URL + "/admin/subscription/downgrade", payload)
      .then(({ data }) => {
        return data;
      });
  },
  ...
  getSubscription(context) {
    return axios.get(BASE_URL + "/admin/subscription").then(({ data }) => {
      console.log(data);
  ...
  getSubscriptionProduct(context, subscription) {
    return axios
      .get(
        BASE_URL + "/admin/subscription_product/" + subscription.plan.product
      )
      .then(({ data }) => {
  ...
  getStorageUsage(context) {
    return axios.get(BASE_URL + "/admin/storage/size").then(({ data }) => {
      context.commit("set", {
        key: "storageUsage",
        value: data,
      });
  ...
  getSubscriptionPlans(context) {
    ...
    return axios
      .get(BASE_URL + "/admin/subscription_plans")
      .then(({ data }) => {
        context.commit("set", {
  ...
  getAdmin(context) {
    ...
    return axios.get(BASE_URL + "/admin").then(({ data }) => {
      const admin = data;
  ...
  createCheckoutSession(context, payload) {
    return axios
      .post(BASE_URL + "/admin/checkout/session", payload)
      .then(({ data }) => {
        return data;
      });
  },
  getCheckoutSession(context, sessionId) {
    return axios
      .get(BASE_URL + "/admin/checkout/session/" + sessionId)
      .then(({ data }) => {
        return data;
      });
  },
  ...
  adminCreateStudentActivity(context, { studentId, text }) {
    ...
    return axios
      .post(BASE_URL + "/admin/student/activity", { studentId, text })
      .then((res) => {
  ...
  getStudentCourse(context, id) {
    return axios.get(BASE_URL + "/student/course/" + id).then((res) => {
      ...
      const promises = res.data.units.map((unit) => {
        return axios
          .get(BASE_URL + "/student/unit/" + unit.id + "/progress")
          .then((res) => ({
            unitId: unit.id,
            progress: res.data,
          }));
  ...
  getFileUrl(context, { format, cardId }) {
    return axios
      .get(BASE_URL + "/admin/card/" + cardId + "/" + format)
      .then(({ data }) => data);
  },
  getSponsorLogo(context, { filename }) {
    return axios
      .post(BASE_URL + "/admin/sponsor/get-logo", { filename })
      .then(({ data }) => data);
  },
  ...
  getStudentCard(context, { courseId, unitId, cardId }) {
    if (!context.activeStudentCourse) {
      return axios.get(BASE_URL + "/student/course/" + courseId).then((res) => {
        const course = res.data;
        const activeUnitIndex = course.units.findIndex(
          (unit) => unit.id === unitId
        );
        const unit = addNextUnitAndCardIds(
          course,
          course.units[activeUnitIndex]
        );
        const activeCardIndex = unit.cards.findIndex(
          (card) => card.id === cardId
        );
      ...
      const activeUnitIndex = context.activeStudentCourse.units.findIndex(
        (unit) => unit.id === unitId
      );
      const unit = addNextUnitAndCardIds(
        context.activeStudentCourse,
        context.activeStudentCourse.units[activeUnitIndex]
      );
      const activeCardIndex = unit.cards.findIndex(
        (card) => card.id === cardId
      );
  ...
  editStudentDetails(context, payload) {
    ...
    return axios
      .put(BASE_URL + "/update-student-details", payload, {
  ...
  editAdminDetails(context, payload) {
    const token = context.state.route.query.token;
    return axios
      .put(BASE_URL + "/update-admin-details", payload, {
        headers: { Authorization: "bearer " + token },
      })
      .then((res) => {
        const card = res.data;
  ...
  editMentorDetails(context, payload) {
    const token = context.state.route.query.token;
    return axios
      .put(BASE_URL + "/update-mentor-details", payload, {
        headers: { Authorization: "bearer " + token },
      })
      .then((res) => {
        const card = res.data;
  ...
  submitStudentCard(context, completed) {
    ...
    return axios.post(`${BASE_URL}/student/card/${cardId}/submit`, {
      completed,
    });
  },
  ...
  getStudentProfile(context, id) {
    ...
    const getStudentProfile = axios
      .get(BASE_URL + "/admin/student/" + id)
      .then((res) => res.data);
    const getStudentActivity = axios
      .get(BASE_URL + "/admin/student/" + id + "/activity")
      .then((res) => res.data);
    const getStudentProgress = axios
      .get(BASE_URL + "/admin/student/" + id + "/progress")
      .then((res) => res.data);
    ...
    return Promise.all([
      ...
      getStudentActivity,
      getStudentProgress,
    ]).then(([student, activity, progress]) => {
  ...
  editBusinessName(context, { businessId, name }) {
    return axios
      .put(BASE_URL + "/api/business/" + businessId, { name })
      .then((res) => {
  ...
  addStudentCourse(context, payload) {
    return axios
      .post(BASE_URL + "/admin/student-course", payload)
      .then((res) => {
        // context.state.socket.emit('notify-student')
  ...
  removeStudentFromCourse(context, payload) {
    return axios
      .delete(BASE_URL + "/admin/student-course", { data: payload })
      .then((res) => {
        // context.state.socket.emit('notify-student')
        context.dispatch("getStudentProfile", payload.studentId);
  ...
  addStudentBusiness(context, payload) {
    ...
    return axios
      .post(BASE_URL + "/admin/student-business", payload)
      .then((res) => {
        context.dispatch("getStudentProfile", payload.studentId);
  ...
  removeStudentFromBusiness(context, payload) {
    return axios
      .delete(BASE_URL + "/admin/student-business", { data: payload })
      .then((res) => {
        // context.state.socket.emit('notify-student')
        context.dispatch("getStudentProfile", payload.studentId);
  ...
  fetchCourse(context, id) {
    return axios.get(BASE_URL + "/admin/course/" + id).then((res) => {
      const course = res.data;
      context.commit("setBreadcrumbs", [
        {
          label: course.name,
  ...
  fetchUnit(context, unitId) {
    ...
    return axios.get(BASE_URL + "/admin/unit/" + unitId).then((res) => {
      const unit = res.data;
      if (!unit) {
      ...
      const activeCardIndex = unit.cards.findIndex(
        (card) => card.id === parseInt(context.state.route.params.cardId)
      );
  ...
  fetchStudentProgress(context, student) {
    return axios
      .get(BASE_URL + "/admin/student/" + student.studentId + "/progress")
      .then((res) => {
  ...
  editUnitSortOrder(context, { courseId, unitId, sortOrder }) {
    return axios
      .put(BASE_URL + "/api/unit/" + unitId, { sortOrder })
      .then((res) => {
  ...
  editCourseName(context, { courseId, name }) {
    return axios
      .put(BASE_URL + "/api/course/" + courseId, { name })
      .then((res) => {
  ...
  editCourseSortOrder(context, { courseId, sortOrder }) {
    return axios
      .put(BASE_URL + "/api/course/" + courseId, { sortOrder })
      .then((res) => {
  ...
  createCard(context, { courseId, unitId, name }) {
    return axios
      .post(BASE_URL + "/admin/unit/" + unitId + "/card", {
        courseId,
        unitId,
        name,
        cardType: "default",
  ...
  createMeetingCard(context, { courseId, unitId, name, time, cardType }) {
    return axios
      .post(BASE_URL + "/admin/unit/" + unitId + "/meetingcard", {
        courseId,
        unitId,
        name,
        time,
  ...
  removeCardAudio(context, { courseId, unitId, cardId }) {
    return axios
      .delete(`${BASE_URL}/admin/card/${cardId}/audio`)
      .then((res) => {
        context.commit("setActiveCardAudio", {
          courseId,
          unitId,
          cardId,
          file: null,
  ...
  removeCardVideo(context, { courseId, unitId, cardId }) {
    return axios
      .delete(`${BASE_URL}/admin/card/${cardId}/video`, {
        data: {
          subscriptionPlan: context.state.subscription.product.name,
        },
      })
      .then((res) => {
        context.commit("setActiveCardVideo", {
          courseId,
          unitId,
          cardId,
          file: null,
  ...
  updateActiveCardQuiz(context, quiz) {
    ...
    const courseIndex = context.state.courses.findIndex(
      (course) => course.id === courseId
    );
    const unitIndex = context.state.courses[courseIndex].units.findIndex(
      (unit) => unit.id === unitId
    );
    const cardIndex = context.state.courses[courseIndex].units[
      unitIndex
    ].cards.findIndex((card) => card.id === cardId);

    const card =
      context.state.courses[courseIndex].units[unitIndex].cards[cardIndex];

    context.commit("setActiveCardQuiz", {
      courseId,
      unitId,
      cardId,
  ...
  addBusinessCourse(context, payload) {
    return axios
      .post(BASE_URL + "/admin/business-course", payload)
      .then((res) => {
  ...
  removeCourseFromBusiness(context, payload) {
    return axios
      .delete(BASE_URL + "/admin/business-course", { data: payload })
      .then((res) => {
        context.dispatch("getBusinessProfile", payload.businessId);
        return res.data;
      });
