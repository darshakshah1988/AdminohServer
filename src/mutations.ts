...
export const mutations = {
  ...
  setStudentProgress(state, { businessId, studentId, progress }) {
    ...
    const studentIndex = state.businesses[businessIndex].students.findIndex(
      (student) => student.id === studentId
    );
    state.businesses[businessIndex].students[
      studentIndex
    ].BusinessStudent.progress = progress;
  },
  ...
  setUnitProgress(state, { courseId, unitId, payload }) {
    const courseIndex = state.student.courses.findIndex(
      (course) => course.id === courseId
    );
    const unitIndex = state.student.courses[courseIndex].units.findIndex(
      (unit) => unit.id === unitId
    );
    assign(state.student.courses[courseIndex].units[unitIndex], {
      progress: payload,
    });
  },
  ...
  setActiveCardQuiz(state, { courseId, unitId, cardId, quiz }) {
    ...
    const unitIndex = state.courses[courseIndex].units.findIndex(
      (unit) => unit.id === unitId
    );
    const cardIndex = state.courses[courseIndex].units[
      unitIndex
  ...
  setActiveStudentProfile(state, student) {
    ...
    state.businesses.forEach((business, businessIndex) => {
      if (business.students) {
        const studentIndex = business.students.findIndex(
          (d) => d.id === student.id
        );
